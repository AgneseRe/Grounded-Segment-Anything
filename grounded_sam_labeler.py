...
class GSAMDatasetLabeler:
    ...
    def _best_matching_concept_label(detected_phrase: str, concept_descriptions: dict) -> Optional[str]:
        ...
        (odd / distractor_a / distractor_b) shares the most words with it.

        Args:
            detected_phrase (str): Phrase returned by Grounding DINO for a given box.
            concept_descriptions (dict): Mapping from concept label ("odd", "a", "b") to
                that concept's object description.

        Returns:
            Optional[str]: The best-matching concept label, or None if there is no overlap
                with any of the descriptions.
        """
        detected_tokens = set(detected_phrase.lower().strip().split())
        if not detected_tokens:
            return None
        
        best_label, best_overlap = None, 0
        for label, description in concept_descriptions.items():
            desc_tokens = set(description.lower().strip().split())
            overlap = len(detected_tokens & desc_tokens)
        ...
        if self.dataset == 'O3':
            ...
            if self.csv_obj_desc_path:
                ...
                concept_descriptions = {
                    "odd": self.concept2desc[odd],
                    "a": self.concept2desc[a],
                    "b": self.concept2desc[b],
                }
            else:
                class_name = f"{odd} . {a} . {b}"
                concept_descriptions = {"odd": odd, "a": a, "b": b}
        ...
        try:
            ...
            for i, mask_tensor in enumerate(masks):
                ...
                if self.dataset == 'O3':
                    ...
                    best_label = self._best_matching_concept_label(detected_phrase, concept_descriptions)
                    is_the_odd = (best_label == "odd")
